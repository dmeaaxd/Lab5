...
public class Remove_lower implements Command {
    ...
    public Remove_lower(CommandHolder commandHolder) {
    ...
    public String execute(CommandInvoker commandInvoker, ArrayList<HumanBeing> collection, String arg) {
        if (arg.isEmpty()) {
            return commandHolder.remove_lower(collection);
        } else {
    ...
    public String getHelp() {
        ...
        final String ANSI_RESET = "\u001B[0m";
        return ANSI_GREEN+"remove_lower"+ANSI_GREEN + ANSI_RESET +" - удалить из коллекции все элементы, меньшие, чем заданный"+ANSI_RESET;
    }
}
