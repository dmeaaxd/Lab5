...
public class Reorder implements Command {
    ...
    public Reorder(CommandHolder commandHolder) {
    ...
    public String execute(CommandInvoker commandInvoker, ArrayList<HumanBeing> collection, String arg) {
        if (arg.isEmpty()) {
            return commandHolder.reorder(collection);
        } else {
    ...
    public String getHelp() {
        ...
        final String ANSI_RESET = "\u001B[0m";
        return ANSI_GREEN+"reorder"+ANSI_GREEN + ANSI_RESET +" - отсортировать коллекцию в порядке, обратном нынешнему"+ANSI_RESET;
    }
}
