...
public class Sort implements Command {
    ...
    public Sort(CommandHolder commandHolder) {
    ...
    public String execute(CommandInvoker commandInvoker, ArrayList<HumanBeing> collection, String arg) {
        if (arg.isEmpty()) {
            return commandHolder.sort(collection);
        } else {
    ...
    public String getHelp() {
        ...
        final String ANSI_RESET = "\u001B[0m";
        return ANSI_GREEN+"sort"+ANSI_GREEN + ANSI_RESET +" - отсортировать коллекцию в естественном порядке"+ANSI_RESET;
    }
}
