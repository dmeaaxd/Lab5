...
public class GroupCountingByImpactSpeed implements Command {
    ...
    public GroupCountingByImpactSpeed(CommandHolder commandHolder) {
    ...
    public String execute(CommandInvoker commandInvoker, ArrayList<HumanBeing> collection, String arg) {
        if (arg.isEmpty()) {
            return commandHolder.group_counting_by_impact_speed(collection);
        } else {
    ...
    public String getHelp() {
        ...
        final String ANSI_RESET = "\u001B[0m";
        return ANSI_GREEN+"group_counting_by_impact_speed"+ANSI_GREEN + ANSI_RESET +" - сгруппировать элементы коллекции по значению поля impactSpeed, вывести количество элементов в каждой группе"+ANSI_RESET;
    }
}
