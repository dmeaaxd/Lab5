...
public class AverangeOfImpactSpeed implements Command {
    ...
    public AverangeOfImpactSpeed(CommandHolder commandHolder) {
    ...
    public String execute(CommandInvoker commandInvoker, ArrayList<HumanBeing> collection, String arg) {
        if (arg.isEmpty()) {
            return commandHolder.averange_of_impact_speed(collection);
        } else {
    ...
    public String getHelp() {
        ...
        final String ANSI_RESET = "\u001B[0m";
        return ANSI_GREEN+"averange_of_impact_speed"+ANSI_GREEN + ANSI_RESET +" - вывести среднее значение поля impactSpeed для всех элементов коллекции"+ANSI_RESET;
    }
}
